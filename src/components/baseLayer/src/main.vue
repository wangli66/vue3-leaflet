<template>
    <div class="l-base-layer">
        <template v-if="Array.isArray(currentLayerOptions)">
            <component :is="layerComp" v-for="(layer,lIndex) in currentLayerOptions" :key="randomNum+'-'+lIndex"
                v-bind="layer"></component>
        </template>
        <template v-else-if="typeof currentLayerOptions=='object'">
            <component :is="layerComp" v-bind="currentLayerOptions" :key="randomNum"></component>
        </template>
        <div class="map-type-wrap" :style="positionClass" v-if="showType && mapTypeList">
            <div class="map-type" @click.stop>
                <div class="map-type-item" v-for="(type,tIndex) of mapTypeList" :key="'map-type-'+tIndex"
                    :class="[type.nameClass, {'active':tIndex==visibleIndex}, type.customClass]"
